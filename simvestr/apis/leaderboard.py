...
def get_ordered_portfolios():
    # returns array of {"porfolio_id","value"}
    # objects ordered by value of the portfolio (closing_balance + investment value)
...
class PortfolioQuery(Resource):
    ...
    @api.response(404, "PortfolioPrice doesn't exist")
    @api.doc(
    ...
    def get(self):
        ...
        p = PortfolioPrice.query.filter(PortfolioPrice.portfolio_id == portfolio_id).order_by(
    ...
    def get(self):
        ...
        if num_ports > 0:
            ...
            for p in db.session.query(Portfolio).join(Portfolio.user).filter(Portfolio.id.in_(users)).all():
                portfolios.append({"id": p.id,
                                   "position": id_value_pos[p.id][1],
                                   "user": p.user.first_name + " " + p.user.last_name,
                                   "name": p.portfolio_name,
                                   "value": id_value_pos[p.id][0]}
                                  )
